...
class Cache {
  ...
  uint32_t* run(armaddr_t address, enum HookMemory::memory_type type, armaddr_t *value)
  {
      ...
      // For 2-way skew associative cache, we have two hashing functions. For the
      // first hash function 1st line is used, for second, 2nd and so on.

      uint32_t f1_index = (2*index) % no_of_sets;
      uint32_t f2_index = (9*index + 3) % no_of_sets;

      // Look for any entry in the Cache line marked by "index"
      for (int i = 0; i < no_of_lines; i++) {
          CacheLine *line;
          if (i == 0)
            line = &sets[f1_index].lines[i];
          else
            line = &sets[f2_index].lines[i];
  ...
  void evict(uint32_t addr, uint32_t offset, uint32_t tag, uint32_t index, enum HookMemory::memory_type type)
  {
    ...
    CacheLine *evicted_line, cuckoo_line;

    /*
     * How to do the cuckoo hashing?
     * - Use the hashing function to first find a index in the cache.
     * - If that index is empty, then put the value there and return.
     * - Otherwise replace the non-empty index with the value and repeat the
     *   process for the replaced index values.
     * - Continue this for a given number of cycles, and then just evict.
     */

    // Flag to check if the iterations ended up in finding an empty place
    // or it had to be evicted.
    // The first block stored will not be dirty. And then the dirty value
    // will also be hopped around.
    bool evicted = false, dirty = false;

    // Cuckoo hashing functions f1 = 2x mod 32 and f2 = 9x + 3 mode 32
    // Run this loop for MAX_CUCKOO_ITER times.
    for (int i = 0; i < MAX_CUCKOO_ITER; i++) {
      // Find the index
      uint32_t f1_index = (2 * index) % no_of_sets;
      uint32_t f2_index = (9 * index + 3) % no_of_sets;
      
      // Compare first bank of index 1 and second bank of index 2
      if (sets[f1_index].lines[0] < sets[f2_index].lines[1])
        evicted_line = &sets[f1_index].lines[0];
      else
        evicted_line = &sets[f2_index].lines[1];

      // If the line to be evicted is a WRITE, then copy the current cache to it
      // and create a copy to continue the loop.
      if (evicted_line->dirty)
        memcpy(&cuckoo_line, evicted_line, sizeof(struct CacheLine));
      else
        evicted = true;
      ...
      evicted_line->dirty = dirty;
      evicted_line->type = type;

      // Prepare for the next iteration
      index = cuckoo_line.blocks.set_bits;
      offset = cuckoo_line.blocks.offset_bits;
      tag = cuckoo_line.blocks.tag_bits;
      dirty = cuckoo_line.dirty;
    }

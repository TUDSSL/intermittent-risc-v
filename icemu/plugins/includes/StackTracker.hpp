...
class StackTracker {
  private:
    icemu::Emulator &_emu;
    bool valid = false;
    address_t min_stack_address = 0; // N.B. the stack grows by decresing the address, so this is the min address
  ...
  public:
    ...
    address_t getStackPointer() {
        return getEmulator().getArchitecture().registerGet(icemu::Architecture::REG_SP);
    }

    void resetMinStackAddress() { 

...
void WarAnalysis::collectWarDependencies() {
  ...
  for (auto &kv : WarDepMap) {
    ...
    PassUtils::ReverseIterateOverInstructions(FirstInst, Write, IterInst);
  }
}
...
bool WarAnalysis::hasUncutPath(CutsTy &Cuts, Instruction *From, Instruction *To) {
  ...
  return PassUtils::ReverseIterateOverInstructions(From, To, IterInst);
}
...
WarAnalysis::PathsTy &WarAnalysis::run() {
   ...
   ****************************************************************************/
  for (auto &W : AllWars) {
    PassUtils::SetInstrumentationMetadata(W.write, "idemp", "idemp_war_wr");
    PassUtils::SetInstrumentationMetadata(W.read, "idemp", "idemp_war_rd");
  }

  // Attach metadata to pre-cut wars
  for (auto &W : PrecutWars) {
    PassUtils::SetInstrumentationMetadata(W.write, "idemp", "idemp_precut_war");
  }

  // Attach metadata to uncut wars
  for (auto &W : UncutWars) {
    PassUtils::SetInstrumentationMetadata(W.write, "idemp", "idemp_uncut_war");
  }
